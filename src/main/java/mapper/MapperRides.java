...
public class MapperRides {
	...
	public Ride inputToRide(final String inputLine) {
		...
		coordFinish.setRow(Integer.valueOf(splittedLine[2]));
		coordFinish.setColumn(Integer.valueOf(splittedLine[3]));

		ride.setStart(coordStart);
		ride.setFinish(coordFinish);
		ride.setEarliestStart(Integer.valueOf(splittedLine[4]));
		ride.setLatestFinish(Integer.valueOf(splittedLine[5]));
